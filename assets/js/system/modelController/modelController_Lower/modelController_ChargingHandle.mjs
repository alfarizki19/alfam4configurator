// Model Controller for Charging Handle (Lower)

import { modelState, objectShowHideSystem, applyTexture, getModelIDFromItemsID, getPartNameFromItemsID } from '../modelController_Core/sketchfabAPI.mjs';

// Import summary functions to reuse data collection
function collectVariants_chargingHandle() {
    const root = window?.part?.chargingHandle;
    const items = [];
    if (!root) return items;
    for (const brandKey in root) {
        const brandNode = root[brandKey];
        const products = brandNode?.products || {};
        for (const productKey in products) {
            const productNode = products[productKey];
            const productTitle = productNode?.productTitle || "";
            const variants = productNode?.variants || {};
            for (const vKey in variants) {
                const v = variants[vKey];
                if (!v?.id) continue;
                items.push({ id: v.id, quantity: Number(v.quantity) || 0, title: productTitle, price: Number(v.price) });
            }
        }
    }
    return items;
}

// Reset Charging Handle Models
export function resetModel_ChargingHandle() {
    const modelIDs = ['chargingHandle001', 'chargingHandle002', 'chargingHandle003', 'chargingHandle004'];
    modelIDs.forEach(modelID => {
        if (modelState.hasOwnProperty(modelID)) {
            modelState[modelID] = 0;
        }
    });
    objectShowHideSystem();
}

// Update Charging Handle Models
export function updateModel_ChargingHandle() {
    const variants = collectVariants_chargingHandle();
    resetModel_ChargingHandle();
    
    variants.forEach(v => {
        if (v.quantity > 0) {
            const modelID = getModelIDFromItemsID(v.id);
            if (modelID && modelState.hasOwnProperty(modelID)) {
                modelState[modelID] = 1;
                applyTexture(v.id);
                console.log(`Charging Handle model shown: ${modelID} with texture: ${v.id}_base`);
            }
        }
    });
    
    objectShowHideSystem();
}

// Handle specific charging handle selection
export function handleChargingHandleSelection(itemsID) {
    const modelID = getModelIDFromItemsID(itemsID);
    
    if (modelID) {
        resetModel_ChargingHandle();
        if (modelState.hasOwnProperty(modelID)) {
            modelState[modelID] = 1;
        }
        applyTexture(itemsID);
        objectShowHideSystem();
        console.log(`Charging Handle selected: ${itemsID} -> ${modelID}`);
    }
}

window.resetModel_ChargingHandle = resetModel_ChargingHandle;
window.updateModel_ChargingHandle = updateModel_ChargingHandle;
window.handleChargingHandleSelection = handleChargingHandleSelection;


